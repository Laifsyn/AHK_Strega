    ...
    Class TargetPath Extends Watchdog_Base { ;; Auto Initialize a SubClass Instance in case there's the setting of a not defined Target Or Watcher
        ...
        refine_Targets(InputType, A_TargetValues*) { ; This is to convert Filetype/Keyword Keywords into a single RegEx if possible. This should solve cases when data is updated on the run
            ...
            For Val in A_TargetValues {
                ...
                {
                    flag := ""
                    ; has_ValidRegExFlag checks if the string contains a valid flag
                    has_ValidRegExFlag := (inputString) => RegExMatch(inputString, "^([imsxADJUXSC]|``a|``r|``n|)+\)", &flagPat)
                    if !RegExMatch(SubPat[1], "^[^\)]{1,14}\)")
                        ; If it doesn't detects a flag, it will add the default flag.
                    {
                        if this.__setting.has("defaultFlag")
                            flag := this.__setting["defaultFlag"], flagType := "instantiated " this.__parentKey
                        else
                            flag := this.__parent.__parent["defaultFlag"], flagType := "Global"
                        if !has_ValidRegExFlag(flag)
                            throw ValueError(Format('Unknown default RegEx Flag in {} defaultFlag', flagType), , flag)
                    } else if !has_ValidRegExFlag(SubPat[1])
                        throw ValueError(Format("Unrecognized regex Flag @Target['{}'], SearchKey of Index {}", this.__parentKey, A_index), , SubPat[1])
                    RegEx.Push(Array(flag SubPat[1], SubPat[2])) ; Strega_Watcher.fileMatch_Logic()'s RegExMatch() expects a string
                    ; msgbox "R:>" SubPat[2] "<"
            ...
            If (this.Type = "Mixed") { ; it matters to know if it's mixed, so It can work with A_LoopFileName without issues
                switch InputType, 0 { ; Case Insensitive - Checks for the original Expected InputType so we add the corresponding regex
                    case "Filetype":
                        temp := "iS)\." temp "$"
                    case "Keyword":
                        temp := "iS)" temp "(?=.*\.[^\.]+)$"
...
Class Strega_Watcher extends Watchdog_Base {
    ...
    doProcedure() {
        ...
        { for key, val in this.storedTimestamp.clone()
            for key2, val2 in val.clone()
                text .= Format("{}\{} = {}s ({})`r`n", key, key2, DateDiff(A_Now, val2.Value, "s"), val2.Value)
    ...
    send_File() {
        ...
        if !(this._loop.regEx is string)
            MsgBox(UDF.getPropsList(this.LF, A_LineNumber, 100)), MsgBox(this._loop.regEx "`r`n" A_LineNumber)
        if 1
            throw ValueError("Finish formatting the destpattern name depending on the matching regex data.")
        try
        { if this._loop.regEx[2] = ""
            msgbox "asds"
        }
        ; if (this._loop.regEx[1] is string) || this._loop.regEx[1][2] = ""
        ;     destName := "*.*"
        ; else
        ;     {
