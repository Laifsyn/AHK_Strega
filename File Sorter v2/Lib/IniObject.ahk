...
class Ini extends Map {
    ...
    __New(Path) {
        sections := StrSplit(IniRead(Path), "`n") ; Retrieves the Ini Sections as a list delimited by "`n", and converts it into an iterable array
    ...
    Load(CreateProperty:=0){ ; This loads the ini file and returns an Map equivalent to the file
        ...
        file_asMap:=Map()
        , FileTimeStamp:= FileGetTime(this.__path,"M")
        , file_asMap.TimeStamp:=FileTimeStamp
        , IniCommentRegEx:=";;(?<Comment>(^;;)*)$"
