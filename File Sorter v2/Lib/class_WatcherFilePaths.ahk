...
class WatcherFiles_Paths extends WatcherFiles_Paths_Internals {
    ...
    __New(parent, namespace, path, encoding?) {
        ...
        this.parent := parent
            ...
            , this.UnprocessedKeywords := Array()
...
class WatcherFiles_Paths_Internals extends Map {
    ...
    IsValidInstance() {
        ...
        Try
            this.DefineProp("initial_map", { Value: JXON.Load(data_json) })
        Catch as Err {
            SetListVars(data_json "`r`n`r`n" (path := Format("{}", this.search_path)), true, "Problem getting data from `r`n" path)
            throw Err
        }
            ...
            if value is Array {
                for ArrIndex, IndexString in value {
                    if this.ConvertKeyword(&IndexString, this.parent["UserDefined"], this.KeywordsExceptions)
                        this.UnprocessedKeywords.push(ArrIndex)
                    value[ArrIndex] := IndexString
                }
            } else if this.ConvertKeyword(&value, this.parent["UserDefined"], this.KeywordsExceptions)
                this.UnprocessedKeywords.push(0)
    ...
    ConvertKeyword(&StringToConvert, keyword_list, keywords_exceptions) {
        ...
        while ((ch := SubStr(StringToConvert, ++pos, 1)) != "") {
            ...
            if ch == "<" {
                if (++keyword["count"]) and capture = true
                ...
                if (--keyword["count"]) = 0 {
                    capture := false
                    old := patt := SubStr(keyword["string"], 1, StrLen(keyword["string"]) - 1) ; This is the captured pattern
                    if this.get_keyword_value(&patt, keyword_list, keywords_exceptions)
                        string_has_unprocessed_keywords += 1
                    if !RegExMatch(patt, "<" old ">") ; This conditional will  filter cases when <keywords_exceptions> triggers
                        && RegExMatch(patt, "<[^>]*>") ; This conditional will convert nested <keywords>
                        ; string_has_unprocessed_keywords -= 1,
                        string_has_unprocessed_keywords += this.ConvertKeyword(&patt, keyword_list, keywords_exceptions)
                    resultString .= patt
                    keyword["string"] := ""
                    continue
                }
    ...
    get_keyword_value(&keyword_string, keyword_list, keywords_exceptions) {
        if RegExMatch(keyword_string, "^\*") || (IsInList(keyword_string, keywords_exceptions))
            return keyword_string := Format("<{}>", Trim(keyword_string, "*"))

        ; if the provided keyword_string exists in keyword_list , it will prioritize the traduction from the list
        if keyword_list.has(keyword_string)
            keyword_string := keyword_list[keyword_string]
        else if value_in_global_context := %keyword_string% ?? "" ; Checks if this is an integrated keyword - A keyword defined in global
            keyword_string := value_in_global_context
        else {
            ; if !keyword_list.has(keyword_string) { ; Commented out in case I find out I needed these
            SplitPath(this.search_path, &filename)
            SetListVars(Format("Defined keys outside of global list:`r`n{}", StrReplace(JXON.Dump(keyword_list, 2), "`n", "`r`n")))
            throw Error(Format('Keyword without definitinion in "{}"`r`nPath: "{}"', filename, this.search_path), , Format('"{}"', keyword_string))
            ; }
            ; keyword_string := keyword_list[keyword_string]
        }
    }
     ...
     * @returns {bool} 
     */
    has_unprocessed_keywords() {
        return !!this.UnprocessedKeywords.Length
    }
    /**
